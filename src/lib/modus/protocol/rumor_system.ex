defmodule Modus.Protocol.RumorSystem do
  ...
  @spec spread_rumor(String.t(), String.t(), String.t(), integer()) ::
          {:ok, map()} | {:skipped, :already_known | :too_degraded}
  ...
  defp maybe_alter_content(content, _accuracy) do
    ...
    alterations = [
      ...
      fn c ->
        String.replace(c, ~r/\b\w+\b/, fn w ->
          if :rand.uniform() < 0.3, do: "something", else: w
        end)
      end
    ]
