defmodule Modus.Simulation.WorldEvents do
  ...
  @event_config %{
    storm:          %{emoji: "🌩️", color: "grey",   duration: 50,  base_radius: 8,  mood_effect: :fear,     mood_delta: -0.15, category: :disaster},
    earthquake:     %{emoji: "🌍", color: "brown",  duration: 20,  base_radius: 12, mood_effect: :fear,     mood_delta: -0.25, category: :disaster},
    meteor_shower:  %{emoji: "☄️",  color: "orange", duration: 30,  base_radius: 6,  mood_effect: :desire,   mood_delta: 0.1,  category: :discovery},
    plague:         %{emoji: "🦠", color: "green",  duration: 80,  base_radius: 10, mood_effect: :sadness,  mood_delta: -0.2,  category: :disaster},
    golden_age:     %{emoji: "✨", color: "gold",   duration: 100, base_radius: 15, mood_effect: :joy,      mood_delta: 0.3,  category: :celebration},
    flood:          %{emoji: "🌊", color: "blue",   duration: 40,  base_radius: 10, mood_effect: :fear,     mood_delta: -0.15, category: :disaster},
    fire:           %{emoji: "🔥", color: "red",    duration: 35,  base_radius: 7,  mood_effect: :fear,     mood_delta: -0.2,  category: :disaster},
    drought:        %{emoji: "🏜️", color: "amber",  duration: 120, base_radius: 15, mood_effect: :sadness,  mood_delta: -0.1,  category: :disaster},
    famine:         %{emoji: "💀🌾", color: "red",  duration: 100, base_radius: 20, mood_effect: :sadness,  mood_delta: -0.25, category: :disaster},
    festival:       %{emoji: "🎉", color: "gold",   duration: 40,  base_radius: 12, mood_effect: :joy,      mood_delta: 0.2,  category: :celebration},
    discovery:      %{emoji: "🗺️", color: "cyan",   duration: 10,  base_radius: 5,  mood_effect: :desire,   mood_delta: 0.15, category: :discovery},
    migration_wave: %{emoji: "🚶", color: "green",  duration: 30,  base_radius: 10, mood_effect: :desire,   mood_delta: 0.05, category: :migration},
    conflict:       %{emoji: "⚔️", color: "red",    duration: 60,  base_radius: 8,  mood_effect: :fear,     mood_delta: -0.2,  category: :disaster}
  }
  ...
  defstruct active_events: [], history: [], last_check_tick: 0,
            scheduled_chains: [], recovery_zones: [], last_event_tick: 0
  ...
  def handle_call({:trigger, event_type, opts}, _from, state) do
    ...
    new_scheduled = state.scheduled_chains ++ Enum.map(chains, fn {type, trigger_tick, sev} ->
      %{type: type, tick: trigger_tick, severity: sev, center: random_center(), source: event_type}
    end)
  ...
  def handle_cast({:tick, tick_number}, state) do
    ...
    new_recovery = for e <- expired, e.config.category == :disaster do
      %{center: e.center, radius: e.radius, start_tick: tick_number,
        duration: e.duration * 2, type: e.type}
    end
    ...
    {ready_chains, pending_chains} = Enum.split_with(state.scheduled_chains, fn c ->
      tick_number >= c.tick
    end)

    {chain_events, chain_scheduled} = Enum.reduce(ready_chains, {[], []}, fn chain, {events, scheds} ->
      ...
      new_scheds = Enum.map(new_chains, fn {type, trigger_tick, sev} ->
        %{type: type, tick: trigger_tick, severity: sev, center: random_center(), source: chain.type}
      end)
    ...
    {new_random_events, new_random_chains} = if rem(tick_number, 50) == 0 do
      ...
      case EventProbability.roll(context) do
        {event_type, severity} ->
          ...
          new_scheds = Enum.map(chains, fn {type, trigger_tick, sev} ->
            %{type: type, tick: trigger_tick, severity: sev, center: random_center(), source: event_type}
          end)
          {[event], new_scheds}
        nil -> {[], []}
    ...
    recovery = (state.recovery_zones ++ new_recovery)
    ...
    {:noreply, %{state |
      ...
      last_event_tick: if(new_random_events != [] or chain_events != [], do: tick_number, else: state.last_event_tick)
    }}
  end
  ...
  defp build_context(tick, state, active_events) do
    season = try do
      ...
      s.name
    catch _, _ -> :spring end

    {population, avg_happiness} = try do
      agents = Registry.select(Modus.AgentRegistry, [{{:"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}])
      pop = length(agents)
      happiness = if pop > 0 do
        total = Enum.reduce(agents, 0.0, fn {_id, pid}, acc ->
          try do
            s = GenServer.call(pid, :get_state, 500)
            acc + (s.conatus_energy || 0.5)
          catch :exit, _ -> acc + 0.5 end
        end)
        total / pop
      else 0.5 end
      {pop, happiness}
    catch _, _ -> {10, 0.5} end

    %{
      season: season,
      avg_happiness: avg_happiness,
      population: population,
      food_ratio: 0.6,  # Simplified
    ...
    for {pid, _id} <- agents do
      try do
        ...
        case event.type do
          ...
          :drought -> GenServer.cast(pid, {:boost_need, :hunger, -8.0 * event.severity})
          :famine -> GenServer.cast(pid, {:boost_need, :hunger, -20.0 * event.severity})
          :festival -> GenServer.cast(pid, {:boost_need, :social, 15.0})
          :conflict -> GenServer.cast(pid, {:boost_need, :rest, -12.0 * event.severity})
          :discovery ->
  ...
  defp apply_terrain_changes(event) do
    ...
    changes = case event.type do
      ...
      :drought -> for x <- (cx-radius)..(cx+radius), y <- (cy-radius)..(cy+radius),
                      in_radius?({x,y}, event.center, radius), do: {{x,y}, :desert}
      _ -> []
    end

    for {pos, terrain} <- changes do
      try do World.paint_terrain(pos, terrain) catch _, _ -> :ok end
    end
  end
  ...
  defp apply_recovery(zone) do
    ...
    spot = {:rand.uniform(radius * 2 + 1) - radius - 1 + cx,
            :rand.uniform(radius * 2 + 1) - radius - 1 + cy}
    try do World.paint_terrain(spot, :grass) catch _, _ -> :ok end
  end
  ...
  defp serialize_event(event) do
    {cx, cy} = event.center
    current_tick = try do Modus.Simulation.Ticker.current_tick() catch _, _ -> 0 end
    %{
      id: event.id,
