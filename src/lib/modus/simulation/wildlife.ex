defmodule Modus.Simulation.Wildlife do
  ...
  def add_plant_regrowth(position, tick),
    do: GenServer.cast(__MODULE__, {:add_regrowth, position, tick})

  @spec tick(non_neg_integer(), atom()) :: :ok
  def tick(tick_number, season \\ :spring),
    do: GenServer.cast(__MODULE__, {:tick, tick_number, season})
    ...
    Enum.reduce(@hunt_rates, animals, fn {predator, prey_map}, acc ->
      ...
      if pred_count > 0 do
        Enum.reduce(prey_map, acc, fn {prey, rate}, acc2 ->
          ...
          kills =
            floor(
              ensure_float(pred_count) * rate * ensure_float(prey_count) /
                max(ensure_float(prey_count), 1.0)
            )
  ...
  def calculate_ecosystem_health(animals) do
    ...
    alive_species =
      Enum.count(animals, fn {species, count} ->
  ...
  def handle_call({:hunt, species}, _from, state) do
    ...
    if count > 0 do
      ...
      {:reply, {:ok, count - 1},
       %{state | animals: new_animals, overhunt_counter: overhunt, ecosystem_health: health}}
    else
  ...
  def handle_call(:serialize, _from, state) do
    {:reply,
     %{
       animals:
         Enum.map(state.animals, fn {k, v} -> {Atom.to_string(k), v} end) |> Enum.into(%{}),
       fishing_spots:
         Enum.map(state.fishing_spots, fn s ->
           {x, y} = s.position
  ...
  def handle_cast({:tick, tick_number, season}, state) do
    ...
    fishing_spots =
      Enum.map(state.fishing_spots, fn spot ->
    ...
    {regrown, still_growing} =
      Enum.split_with(state.plant_regrowth, fn {_pos, planted_tick} ->
  ...
  def handle_cast({:kill_animals_in_area, species_list, count}, state) do
    animals =
      Enum.reduce(species_list, state.animals, fn species, acc ->
